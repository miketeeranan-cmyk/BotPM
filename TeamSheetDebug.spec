# PyInstaller spec for a debug build of the Windows desktop app -- same as
# TeamSheet.spec but with a console window attached so startup crashes and
# stray output are visible instead of silently failing. Built only by CI on
# a Windows runner -- PyInstaller cannot cross-compile, so this cannot
# produce a working exe from macOS.
#
...
exe = EXE(
    ...
    upx=False,
    runtime_tmpdir=None,
    console=True,  # debug build: keep the console window for crash output
